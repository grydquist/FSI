...
SUBROUTINE gstiffmat(msh,kab,f,KABg,Fg,iel)
...
INTEGER :: a,i,c
INTEGER, ALLOCATABLE :: locglo(:)

! Current local element I'm trying to incorporate into big one
el => msh%el(iel)

! locglo takes the dof at a given node and gives location in big matrix
ALLOCATE(locglo(el%eNoN*el%dof))
DO a = 1,el%eNoN
    DO i = 1,el%dof
